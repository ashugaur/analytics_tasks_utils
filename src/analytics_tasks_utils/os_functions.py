...
def get_zip_file_structure(zip_ref, path=""):
    file_names = [f.filename for f in zip_ref.filelist if f.filename.startswith(path) and f.filename != path]
    dir_names = set()
    ...
    file_names_in_dir = [f for f in file_names if Path(f).parent.as_posix() + "/" == path or (path == "" and "/" not in f)]
...
def visualize_zip_file(zip_path, output_file):
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref, open(output_file, 'w', encoding='utf-8') as f:
            root = DisplayableZipPath("", None, False)

            def make_tree(parent_path, parent_displayable):
                dir_names, file_names_in_dir = get_zip_file_structure(zip_ref, parent_path)
                ...
                for dir_name in dir_names:
                    ...
                    dir_displayable = DisplayableZipPath(dir_name_short, parent_displayable, count == len(dir_names) + len(file_names_in_dir))
                ...
                for file_name in file_names_in_dir:
                    ...
                    file_displayable = DisplayableZipPath(file_name_short, parent_displayable, count == len(dir_names) + len(file_names_in_dir))
